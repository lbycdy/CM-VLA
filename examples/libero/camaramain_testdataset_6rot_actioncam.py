...
def cam_action_to_base(action_cam, task_suite_name):
    ...
    pose_cam = np.asarray(action_cam[:6], dtype=np.float32)
    gripper = float(action_cam[6]) if len(action_cam) > 6 else 0.0
...
def main1():
    ...
    for file in tqdm.tqdm(parquet_files, desc="回放任务"):
        ...
        for i, row in enumerate(df.itertuples(index=False)):
            ...
            if hasattr(row, "action_cam") and row.action_cam is not None:
                action_vec = cam_action_to_base(np.array(row.action_cam, dtype=np.float32), ep_suite)
