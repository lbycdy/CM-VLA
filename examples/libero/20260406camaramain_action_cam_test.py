...
def get_T_cam_to_base(task_suite_name):
    matrices = {
        "libero_10":     np.array([
        [ 0.14256865,  0.28905121, -0.94663804,  0.73252114],
        [-0.86229807,  0.50580426,  0.02457821, -0.26871372],
        [ 0.48591792,  0.81278007,  0.32136013,  0.62953593],
        [ 0.        ,  0.        ,  0.        ,  1.        ]
    ]),
        "libero_spatial": np.array([
        [ 0.03476975,  0.83236956, -0.55312926,  0.40495509],
        [-0.74115933,  0.39275834,  0.54444809, -0.66984291],
        [ 0.67042815,  0.39102659,  0.63057458,  0.21977148],
        [ 0.        ,  0.        ,  0.        ,  1.        ]
    ]),
...
class Args:
    ...
    compare_gt: bool = False              # 是否对比标签动作
    ...
    #################################################################################################################
    # === 新增：从 npy 回放动作（不用 infer）===
    #################################################################################################################
    action_npy_path: str = "/home/lbycdy/work/camerapi/episode_000007_actions.npy"  # 你的动作文件，支持 [T,7] 或 [T,1,7]
    action_frame: str = "camera"          # "camera" 表示 npy 在相机系；"base" 表示已在基座系
    T_cam_to_base_path: str = ""          # 可选：4x4 矩阵文件路径（.npy 或 文本），优先级低于 flat
    T_cam_to_base_flat: Tuple[float, ...] = ()  # 可选：16 个数（行优先）组成 4x4 变换矩阵



def _load_action_sequence(action_npy_path: str) -> np.ndarray:
    """Load action sequence from .npy. Accepts [T,7], [T,1,7], or [7]. Returns float32 [T,7]."""
    arr = np.load(os.path.expanduser(action_npy_path))
    arr = np.asarray(arr)
    if arr.ndim == 1 and arr.shape[0] == 7:
        arr = arr[None, :]
    elif arr.ndim == 3 and arr.shape[1] == 1 and arr.shape[2] == 7:
        arr = arr[:, 0, :]
    if not (arr.ndim == 2 and arr.shape[1] == 7):
        raise ValueError(f"action npy must be [T,7] or [T,1,7] (or single [7]); got {arr.shape}")
    return arr.astype(np.float32, copy=False)


def _load_T_cam_to_base(args) -> np.ndarray:
    """Load cam->base 4x4 transform. Priority: flat(16) > path > builtin by task_suite_name."""
    flat = getattr(args, "T_cam_to_base_flat", ()) or ()
    if len(flat) > 0:
        if len(flat) != 16:
            raise ValueError(f"T_cam_to_base_flat must have 16 numbers, got {len(flat)}")
        return np.asarray(flat, dtype=np.float32).reshape(4, 4)

    p = (getattr(args, "T_cam_to_base_path", "") or "").strip()
    if p:
        ...
        if T.shape != (4, 4):
            raise ValueError(f"T_cam_to_base from {p} must be shape (4,4), got {T.shape}")
        return T

    return get_T_cam_to_base(getattr(args, "task_suite_name"))
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    # Save preprocessed image for replay video                    # Save preprocessed image for replay video
                    replay_images.append(img)

                    # ====== 从你提供的 npy 动作序列逐帧回放（不用 infer）======
                    if t == args.num_steps_wait:
                        action_seq = _load_action_sequence(args.action_npy_path)
                        action_ptr = 0
                        T_cam_to_base = _load_T_cam_to_base(args)

                    if action_ptr >= len(action_seq):
                        logging.info("Action npy exhausted. Ending rollout.")
                        break

                    action_row = action_seq[action_ptr]  # (7,)
                    action_ptr += 1

                    if args.action_frame.lower() == "camera":
                        action_base = cam_delta_to_base_delta(action_row, T_cam_to_base)
                    elif args.action_frame.lower() == "base":
                        action_base = np.asarray(action_row, dtype=np.float32)
                    else:
                        raise ValueError(f"Unknown action_frame: {args.action_frame}. Use 'camera' or 'base'.")
