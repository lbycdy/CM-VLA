...
# 只保存 action 序列（每行 7 维 action：dx dy dz dRx dRy dRz gripper）
ACTION_PATH = os.path.join(SAVE_ROOT, "actions.txt")

# ✅ 按 B 键保存当前 RGB（agentview + wrist）
SNAPSHOT_DIR = os.path.join(SAVE_ROOT, "snapshots")
...
def _get_rgb_from_obs(obs, cam_name):
    key = _get_obs_key(obs, cam_name, "image") or f"{cam_name}_image"
    if key not in obs:
        return None, key
    img = _to_uint8(obs[key])
    if FLIP_UD:
        img = np.flipud(img)
    return img, key


def save_snapshot_pair(obs, snap_idx):
    """按一次 B：保存当前 agentview + wrist 两张 RGB 图。"""
    if obs is None:
        print("[warn] snapshot skipped: obs is None")
        return

    ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]
    cam_img, cam_key = _get_rgb_from_obs(obs, "agentview")
    wrist_img, wrist_key = _get_rgb_from_obs(obs, "robot0_eye_in_hand")

    saved_any = False

    if cam_img is None:
        print(f"[warn] snapshot: missing agentview image (tried key={cam_key})")
    else:
        cam_path = os.path.join(SNAPSHOT_DIR, f"{ts}_{snap_idx:06d}_agentview.png")
        _save_png(cam_path, cam_img)
        print("[save] agentview:", os.path.abspath(cam_path))
        saved_any = True

    if wrist_img is None:
        print(f"[warn] snapshot: missing wrist image (tried key={wrist_key})")
    else:
        wrist_path = os.path.join(SNAPSHOT_DIR, f"{ts}_{snap_idx:06d}_wrist.png")
        _save_png(wrist_path, wrist_img)
        print("[save] wrist   :", os.path.abspath(wrist_path))
        saved_any = True

    if not saved_any:
        keys_preview = list(obs.keys())[:30]
        print("[debug] obs keys preview:", keys_preview)


def save_step_artifacts(obs, sim, step_idx, action):
    """只追加保存 action 序列（不再保存 RGB/Depth/EEF state）。"""
    a = np.asarray(action, dtype=float).reshape(-1)
    if a.size != 7:
        raise ValueError(f"Expected action dim=7, got {a.size}")
    with open(ACTION_PATH, "a", encoding="utf-8") as f:
        f.write(" ".join(f"{float(v):.8f}" for v in a) + "\n")
...
# ✅ 一定要接住 reset() 返回的 obs，后面按 B 才能保存
obs = env.reset()
last_obs = obs
...
# =========================
# 3) GLFW 键盘窗口（NO_API，无 OpenGL context）
# =========================
if not glfw.init():
...
keyboard_window = glfw.create_window(240, 120, "Keyboard Control (focus me)", None, None)
...
print("J/L/I/K/U/O: 旋转 | Z/X: 合拢/张开")
print("B: 保存当前 agentview+wrist 两张图")
print("Q: 退出\n")
...
try:
    while not glfw.window_should_close(keyboard_window):
        ...
        # ✅ 处理 “按一下 B 就保存一次”（边沿触发：从松开->按下）
        b_down = (glfw.get_key(keyboard_window, glfw.KEY_B) == glfw.PRESS)
        if b_down and (not prev_b_down):
            snap_idx += 1
            save_snapshot_pair(last_obs, snap_idx)
        prev_b_down = b_down
        ...
        if np.any(action[:6] != 0):  # 检查前6个元素是否非零
            ...
            # 没有移动指令：不 step，last_obs 保持最新即可（按 B 保存的是 last_obs）
            pass

        # 是否保存 action：默认只在“你确实输入动作”的那步保存
        has_input = not np.array_equal(action, action_copy)
        if (not SAVE_ONLY_WHEN_INPUT) or has_input:
            step_idx += 1
            # 用 last_obs 防止 obs 未定义
            save_step_artifacts(last_obs, sim, step_idx, action)
            print(f"[save {step_idx:06d}] action={np.array2string(action, precision=3, suppress_small=True)}")
