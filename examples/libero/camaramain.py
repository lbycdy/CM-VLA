...
def cam_action_to_base(action_cam, task_suite_name):
    """Convert 7D action from camera frame to base frame."""
    T_cam_to_base = get_T_cam_to_base(task_suite_name)
    T_base_to_cam = np.linalg.inv(T_cam_to_base)
    pose_cam = action_cam[:6]
    gripper = action_cam[6]
    T_cam_ee = pose6_to_T(pose_cam)
    T_base_ee = T_cam_to_base @ T_cam_ee
    pose_base = T_to_pose6(T_base_ee)
    return np.concatenate([pose_base, [gripper]]).astype(np.float32)
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    action_base = cam_action_to_base(action_cam, args.task_suite_name)

                    # print("\n[DEBUG] action_cam:", action_cam)
                    # print("[DEBUG] R_cam_to_base:\n", R_cam_to_base)
                    # print("[DEBUG] R_base_to_cam (transpose):\n", R_base_to_cam)
                    # print("[DEBUG] lin_base (converted):", lin_base)
                    # print("[DEBUG] ang_base (converted):", ang_base)
                    # exit()
