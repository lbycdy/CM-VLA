...
class Args:
    ...
    #################################################################################################################
    # === 新增：从文件回放动作（不用 infer）：.txt/.csv 或 .npy ===
    #################################################################################################################
    action_npy_path: str = "/home/lbycdy/work/camerapi/examples/libero/recordings/20251212_113122/actions.txt"  # 动作文件：支持 .txt/.csv(每行7个数) 或 .npy([T,7]/[T,1,7])
    action_frame: str = "base"          # "camera" 表示 npy 在相机系；"base" 表示已在基座系
    T_cam_to_base_path: str = ""          # 可选：4x4 矩阵文件路径（.npy 或 文本），优先级低于 flat
    T_cam_to_base_flat: Tuple[float, ...] = ()  # 可选：16 个数（行优先）组成 4x4 变换矩阵



def _load_action_sequence(action_path: str) -> np.ndarray:
    """Load action sequence from a file.

    Supports:
      - .npy: [T,7], [T,1,7], or single [7]
      - .txt/.csv: whitespace or comma separated, each row 7 floats

    Returns float32 array with shape [T,7].
    """
    p = os.path.expanduser(action_path)
    ext = os.path.splitext(p)[1].lower()

    if ext == ".npy":
        arr = np.load(p)
    else:
        # txt/csv (recorded actions.txt is whitespace-separated)
        try:
            arr = np.loadtxt(p, dtype=np.float32)  # whitespace
        except Exception:
            arr = np.loadtxt(p, dtype=np.float32, delimiter=",")  # fallback: csv

    arr = np.asarray(arr)

    if arr.size == 0:
        raise ValueError(f"Empty action file: {p}")
    ...
    if not (arr.ndim == 2 and arr.shape[1] == 7):
        raise ValueError(f"action file must be [T,7] (or [T,1,7] / single [7]); got {arr.shape} from {p}")
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            while t < max_steps + args.num_steps_wait:
                try:
                    ...
                    # ====== 从文件动作序列逐帧回放（不用 infer）======
                    if t == args.num_steps_wait:
                        action_path = os.path.expanduser(args.action_npy_path)
                        action_ext = os.path.splitext(action_path)[1].lower()

                        action_seq = _load_action_sequence(action_path)
                        action_ptr = 0

                        # 兼容旧的 .npy 动作：沿用你原来的缩放；
                        # actions.txt（键盘录制得到的文本）默认不缩放，直接回放。
                        if action_ext == ".npy":
                            action_scale_pos = 100.0
                            action_scale_rot = 10.0
                        else:
                    ...
                    if action_ptr >= len(action_seq):
                        logging.info("Action sequence exhausted. Ending rollout.")
                        break
                    ...
                    action_base[:3] *= action_scale_pos
                    action_base[3:6] *= action_scale_rot
                    # 缓存基座系预测动作
